...
**类的内部类**。有 **static 修饰**的内部类。不能直接访问外部类的实例成员。

语法：`外部类名.内部类名 名字 =  内部类名();`
...
本质是**一个子类**，使用的地方，会返回一个实例对象。通常作为一个实参传给方法，例如：[[02-进阶/14-高级技术#4.2. 生成动态代理对象？\|动态代理创建代理对象]]。

语法：
```java
// 方便创建子类对象的语法。 联想到类lambda式子
new 类或接口(参数){
    // 方法的重写
    @Override
    ...
...
[[02-进阶/02-面向对象编程进阶-2#3. 常量\|常量必须初始化]]

语法
```java
修饰符 enum 枚举类 {
	//枚举的第一行只能是 合法标识符用逗号隔开，分割好结束。
	//这些名字，本质是一个数据类型是枚举类，的常量。
    名字1,名字2,名字3等; 
...
- 枚举类的第一行只能写名字，他们都是常量。从第二行开始可以写其他的成员。
